...
public class FireballBehavior implements SpellBehavior, Serializable {

    // Pattern Singleton
    private static FireballBehavior instance;
    private FireballBehavior(){}

    public static FireballBehavior getInstance(){
        if(instance == null) instance = new FireballBehavior();
        return instance;
    }

    transient Scanner scanner = new Scanner(System.in);

    /** Throws a fireball to a tower or to an enemy */
    public void cast(Spell s, Player ally, User enemy){

        System.out.println("\nVuoi lanciare la palla di fuoco sulla torre o su un nemico?");
        ...
        switch(scanner.next()){
            ...
            System.out.println("\nLa palla di fuoco colpisce la torre avversaria infliggendo "+s.getParam()+" danni.");
            break;
            ...
            System.out.println("\nLa palla di fuoco colpisce il bersaglio infliggendo "+s.getParam()+" danni.");
    ...
    public void cast(Spell s, Computer ally, User enemy){
        ...
        switch(random.nextInt(2)){
            case 0:
            enemy.getTower().setHp(enemy.getTower().getHp()-s.getParam());
            System.out.println("\nIl Computer lancia una palla di fuoco infliggendo "+s.getParam()+" danni alla tua torre.");
            break;
            ...
            if(GameServices.checkBG(enemy)){
                ...
                enemy.getBg().getMap().get(pos).setHp(enemy.getBg().getMap().get(pos).getHp()-s.getParam());
                System.out.println("\nIl Computer lancia una palla di fuoco infliggendo "+s.getParam()+" danni al tuo "+enemy.getBg().getMap().get(pos).getName());
                GameServices.remainingDamage(enemy.getBg().getMap().get(pos), enemy);
            }
        }
    }
